...
module.exports = async (getInstanceQueue, uuid, event) => {
  ...
  const messageData = {
    ...
    msgGroupId: `cancellationOrder.updateCredit-${event.cpgId}-${event.countryId}-${event.organizationId}-${event.orderId}-${event.clientId}`,
    msgDedupId: uuidv4(),
  };
