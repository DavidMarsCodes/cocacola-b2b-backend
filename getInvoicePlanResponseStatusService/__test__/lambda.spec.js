...
describe('Lambda Function', () => {
  it('You should get records from the dynamoDB database via a transactionId.', async () => {
    ...
    const getTableName = jest.fn().mockResolvedValue({ tableNameError: 'invoicePlanToUpsertErrors' });
    ...
    const { getInvoicePlanStatus } = handler({ awsRepositories, getTableName, getErpConfig, erpManager, res });
    await getInvoicePlanStatus(event);

    expect(awsRepositories.Repository.get.mock.calls).toEqual([
      [
        'invoicePlanToUpsertErrors',
        { transactionId: '16fefcc2-28e3-11eb-adc1-0242ac120002' },
      ],
    ]);
  ...
  it('It should throw an error because you can not get the data in DynamoDB.', async () => {
    ...
    const { getInvoicePlanStatus } = handler({ awsRepositories, getTableName, res });
    await getInvoicePlanStatus(event);
