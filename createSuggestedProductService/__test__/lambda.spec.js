...
describe('Lambda Function', () => {
  it('You should save an order in the dynamoDB database.', async () => {
    ...
    const experts = { product: { validateCreateSuggestedProduct: jest.fn() } };
    ...
    const { createSuggestedProduct } = handler({ awsRepositories, getTableName, experts, res });
    await createSuggestedProduct(event);

    expect(experts.product.validateCreateSuggestedProduct.mock.calls).toMatchSnapshot([
      [
        event,
      ],
    ]);
  ...
  it('It should answer a validation error in case any of the required parameters are invalid.', async () => {
    ...
    const experts = {
      product: {
        validateCreateSuggestedProduct: jest.fn(data => {
          throw {
            customError: true,
  ...
  it('It should throw an error because of not being able to store the data in DynamoDB.', async () => {
    ...
    const { createSuggestedProduct } = handler({ awsRepositories, experts, res });
    await createSuggestedProduct(event);
