...
describe('Lambda Function', () => {
  it('You should get records from the dynamoDB database via a transactionId.', async () => {
    ...
    const getTableName = jest.fn().mockResolvedValue({ tableNameError: 'devUpsertShippingPriceErrors' });
    ...
    const { getShippingPriceResponseStatus } = handler({ awsRepositories, getTableName, getErpConfig, erpManager, res });
    await getShippingPriceResponseStatus(event);

    expect(getTableName.mock.calls).toEqual([ [ ] ]);
    expect(awsRepositories.Repository.get.mock.calls).toEqual([
      [
        'devUpsertShippingPriceErrors',
        { transactionId: '16fefcc2-28e3-11eb-adc1-0242ac120002' },
      ],
    ]);
  ...
  it('It should throw an error because you can not get the data in DynamoDB.', async () => {
    ...
    const { getShippingPriceResponseStatus } = handler({ awsRepositories, getTableName, getErpConfig, erpManager, res });
    const result = await getShippingPriceResponseStatus(event);
