...
describe('Lambda Function', () => {
  it('You should get records from the dynamoDB database via a transactionId.', async () => {
    ...
    const getTableName = jest.fn().mockResolvedValue({ tableNameError: 'devUpsertProductErrors' });
    ...
    const { getProductResponseStatusService } = handler({ awsRepositories, getTableName, getErpConfig, erpManager, res });
    await getProductResponseStatusService(event);

    expect(getTableName.mock.calls).toEqual([ [ ] ]);
    expect(awsRepositories.Repository.get.mock.calls).toEqual([
      [
        'devUpsertProductErrors',
        { transactionId: '16fefcc2-28e3-11eb-adc1-0242ac120002' },
      ],
    ]);
